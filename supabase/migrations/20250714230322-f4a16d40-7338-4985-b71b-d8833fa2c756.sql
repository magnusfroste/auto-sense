-- Update the cron job to use correct function name and remove is_active filter
SELECT cron.unschedule('vehicle-polling-cron');

-- Create a new cron job to poll all vehicles every 2 minutes
SELECT cron.schedule(
  'vehicle-polling-cron-updated',
  '*/2 * * * *', -- Every 2 minutes
  $$
  SELECT
    net.http_post(
